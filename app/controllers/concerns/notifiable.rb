module Notifiable
  ...
  def notification_for_new_lesson(recipient, lesson)
    ...
    create_notification(recipient, "Lesson submitted for Approval", message, lesson_url(lesson))
  end
  ...
  def notification_for_lesson_approval_change(recipient, lesson)
    ...
    create_notification(recipient, "Lesson Approval changed", message, lesson_url(lesson))
  end
  ...
  def notification_for_new_lesson_proposal(recipient, lesson_version)
    message = "Your proposal <a href='#{lesson_version_url(lesson_version)}'>#{lesson_version.name}</a> has been submitted \
              for approval. You'll be notified once it's been reviewed."
    create_notification(recipient, "Lesson update proposal submitted", message, lesson_version_url(lesson_version))
  end
  ...
  def notification_for_lesson_version_approval_change(recipient, lesson_version)
    ...
    create_notification(recipient, "Lesson Version Approval changed", message, lesson_version_url(lesson_version))
  end
  ...
  def notification_for_new_collection(recipient, collection)
    message = "Your new Collection <a href='#{collection_url(collection)}'>#{collection.name}</a> has been submitted \
              for approval. You'll be notified once it's been reviewed."
    create_notification(recipient, "Collection submitted for Approval", message, collection_url(collection))
  end
  ...
  def notification_for_collection_approval_change(recipient, collection)
    ...
    create_notification(recipient, "Collection Approval changed", message, collection_url(collection))
  end

  def create_notification(recipient, name, description, source = nil)
    Notification.create(recipient: recipient, name: name, description: description, source: source)
  end
end
