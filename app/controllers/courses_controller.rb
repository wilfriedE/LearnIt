class CoursesController < ApplicationController
  before_action :authenticate_user!, except: %i[index show viewing]
  ...
  def viewing
    @course = Course.find(params[:id])
    @course_lesson = @course.course_lessons.find_by(position: params[:position])
  ...
  def update
    ...
    respond_to do |format|
      if @course.update_attributes(course_params)
        unless @course.contributors.include?(current_user)
  ...
  def course_params
    params.require(:course).permit(:name, :description,
                                   course_lessons_attributes: %i[id _destroy position lesson_id],
  ...
  def new_course_activity(course)
    ModTicketActivity.create_ticket("New Course: ##{course.id} available.",
                                    "#{course.name} was recently created awaiting approval", [course])
    UserFeedActivity.create_user_notification(current_user, "New Course: ##{course.id} submitted for approval.",
                                              "Thank you for contributing #{course.name}. It has been submitted for review by moderators.", [course])
  end

  def updated_course_activity(course)
    ModTicketActivity.create_ticket("Course: ##{course.id} updated.",
                                    "#{course.name} was recently updated by #{current_user.nickname}", [course])
    course.contributions.each do |contributor|
      UserFeedActivity.create_user_notification(contributor, "Course: ##{course.id} was recently updated.",
                                                "#{course.name}, has recently been updated.", [course])
    end
  end
end
