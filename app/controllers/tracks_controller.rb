class TracksController < ApplicationController
  ...
  def viewing
    @track = Track.find(params[:id])
    @track_courses = @track.track_courses.find_by(position: params[:position])
  ...
  def update
    ...
    respond_to do |format|
      if @track.update_attributes(track_params)
        unless @track.contributors.include?(current_user)
  ...
  def track_params
    params.require(:track).permit(:name, :description,
                                  track_courses_attributes: %i[id _destroy position course_id],
  ...
  def new_track_activity(track)
    ModTicketActivity.create_ticket("New Track: ##{track.id} available.",
                                    "#{track.name} was recently created awaiting approval", [track])
    UserFeedActivity.create_user_notification(current_user, "New Track: ##{track.id} submitted for approval.",
                                              "Thank you for contributing #{track.name}. It has been submitted for review by moderators.", [track])
  end

  def updated_track_activity(track)
    ModTicketActivity.create_ticket("Track: ##{track.id} updated.",
                                    "#{track.name} was recently updated by #{current_user.nickname}", [track])
    track.contributions.each do |contributor|
      UserFeedActivity.create_user_notification(contributor, "Track: ##{track.id} was recently updated.",
                                                "#{track.name}, has recently been updated.", [track])
    end
  end
end
